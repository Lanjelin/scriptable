...
async function createWidget() {
  ...
  function pickAndChoose(activities, values, r){
    ...
    const suffix = {activities: "★", distance: "km", duration: "min", calories: "kcal", activedays: "",}
        ...
        for (let value of values){
          ...
          if (value.includes("padding")){
            ...
            if (value == "distance"){
              ...
              console.log(value+" not supported yet, but please make an issue/suggestion on GitHub 😊");
              err = true
            }
            if (!err){
...
async function getActivities(data, timeFrame){
  ...
  for (let activity of data["activityList"]){
    if (withinTimeframe(timeFrame, activity["startTimeLocal"])){
      ...
      if (!res.total.activedays.includes(getDayNum(activity["startTimeLocal"]))){
      ...
      if (workoutID.includes(activity["activityType"]["typeId"])){
        ...
        if (!res.workout.activedays.includes(getDayNum(activity["startTimeLocal"]))){
        ...
        if (!res.walk.activedays.includes(getDayNum(activity["startTimeLocal"]))){
        ...
        if (!res.run.activedays.includes(getDayNum(activity["startTimeLocal"]))){
        ...
        if (!res.bike.activedays.includes(getDayNum(activity["startTimeLocal"]))){
